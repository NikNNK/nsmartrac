                              ...
                              <table id="expensesCheckItemTable" class="table table-striped table-bordered" style="width:100%;margin-top: 20px;">
                                 ...
                                 <tbody>
                                    <tr>
                                       ...
                                       <td>
                                          <div>
                                              <select name="item[]" class="form-control expenseServiceRate" id="ProductServiceAddNew1" data-id="1">
                                                <option value="">Select Product / Service</option>
                                                <?php foreach ($list_product_services as $_list_product_services): ?>
                                                <option value="<?php echo $_list_product_services->id?>" rate="<?php echo $_list_product_services->rate?>"><?php echo $_list_product_services->title;?></option>
                                                <?php endforeach;?>
                                             </select>
                                          </div>
                                       </td>
...
$(document).ready(function(){
   ...
   $(document).on('click', '.add-check-item-line', function() {
      ...
      var html = '';
      $.ajax({
         url : "<?php echo base_url('accounting/get_product_services_data');?>",
         type : "GET",
         dataType : 'json',
         success: function(data){
            var html = '<option value="">Select Product / Service</option>';
            var i;
            for(i=0; i<data.list_product_services.length; i++){
               html += '<option value='+data.list_product_services[i].id+' rate='+data.list_product_services[i].rate+'>'+data.list_product_services[i].title+'</option>';
            }
            var row = '<tr><td>'+rowCount+'</td><td><div><select name="item[]" class="form-control expenseServiceRate" id="ProductServiceAddNew'+i+'" data-id="'+rowCount+'">'+html+'</select></div></td><td><input type="text" name="product_description[]" class="form-control" id="productDescription'+rowCount+'" data-id="'+rowCount+'"></td><td><input type="text" name="quantity[]" class="form-control ProductQuantity" id="ProductQuantity'+rowCount+'" data-id="'+rowCount+'"></td><td><input type="text" name="rate[]" class="form-control productRate" id="productRate'+rowCount+'" readonly="readonly" data-id="'+rowCount+'"></td><td><input type="text" name="product_amount[]" class="form-control expenseCheckItemAmount" id="productAmount'+rowCount+'" data-id="'+rowCount+'"></td><td style="text-align: center"><a href="#" class="remove-check-item-row"><i class="fa fa-trash"></i></a></td></tr>';
            $("#expensesCheckItemTable tbody").append(row);
         },
      });
   });

    $(document).on('change', '.expenseServiceRate', function() {
        var itemId = $(this).val();
        var id = $(this).attr("data-id");
        $("#ProductQuantity"+id).val(1);
        $("#productRate"+id).val($('#ProductServiceAddNew'+id+' option:selected').attr('rate'));
        check_item_total();
        check_total();
        getproductAmount(id);
   });
   
    function getproductAmount(unique_id){
       var ProductQuantity = $("#ProductQuantity"+unique_id).val();
       var productRate = $("#productRate"+unique_id).val();
       
       var productAmount = parseFloat(ProductQuantity) * parseFloat(productRate);
       
       $("#productAmount"+unique_id).val(productAmount);
        check_item_total();
   }
